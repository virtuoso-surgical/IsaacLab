...
class VirtuosoTissueEnvCfg(TissueEnvCfg):
    def __post_init__(self):
        ...
        self.scene.object = AssetBaseCfg(
            prim_path="{ENV_REGEX_NS}/Object",
            init_state=AssetBaseCfg.InitialStateCfg(pos=[0.077, 0.0, 0.007], rot=[0, 0, 0, 1]),
            # init_state=AssetBaseCfg.InitialStateCfg(pos=[1.094, 0.0, 0.0060], rot=[0, 0, 0, 1]),
            spawn=UsdFileCfg(
                usd_path=f"{ISAACLAB_ASSETS_DATA_DIR}/Props/Skin/skin-best.usd",
                scale=(0.06, 0.035, 0.0005),
                ),
        )
